...
// ─── Constantes ───────────────────────────────────────────────────────────────
// Slots de 30 min: cada hora tiene 2 slots → [8.0, 8.5, 9.0, 9.5, ...]
const SLOTS     = Array.from({length:21}, (_, i) => 8 + i * 0.5); // 8:00 a 18:00
...
const HORA_H    = SLOT_H * 2; // px por hora completa = 60px
...
// slot = número decimal ej 8.5 → hora=8, min=30
...
const esPasadoSlot = (dateStr, slot) => {
  const hoy = new Date();
...
const ModalAgendar = ({slots, onClose, onConfirm, primary, secondary, colors, sp, br, sh, typo})=>{
  ...
  const handleConfirm = async ()=>{
    ...
    try {
      await onConfirm({pacienteId,tipoCitaId,notas,slots});
      setExitoso(true);
      ...
      <div style={{background:colors?.neutral?.[0]||'#fff',borderRadius:br?.xl||'12px',width:'100%',maxWidth:'560px',maxHeight:'92vh',overflow:'hidden',display:'flex',flexDirection:'column',boxShadow:'0 24px 48px rgba(0,0,0,0.2)',animation:'slideUp 0.2s ease'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:sp?.sm||'8px'}}>
            ...
            <div>
              ...
              <p style={{margin:0,fontSize:fs(typo,'xs'),color:colors?.neutral?.[500]||'#6b7280'}}>
                {slots.length} slot{slots.length!==1?'s':''} · {slots.length*30} min reservados
              </p>
            </div>
          </div>
          <button onClick={()=>!guardando&&onClose()} style={{background:'none',border:'none',cursor:'pointer',color:colors?.neutral?.[400]||'#9ca3af',display:'flex',padding:'4px',borderRadius:br?.full||'9999px'}}>
            <MdClose size={22}/>
          </button>
        </div>
        ...
        <div style={{padding:`${sp?.sm||'8px'} ${sp?.xl||'32px'}`,borderBottom:`1px solid ${colors?.neutral?.[100]||'#f3f4f6'}`,background:colors?.neutral?.[50]||'#f9fafb',display:'flex',flexWrap:'wrap',gap:sp?.xs||'4px',flexShrink:0}}>
          {slots.map((s,i)=>(
            <span key={i} style={{display:'inline-flex',alignItems:'center',gap:'4px',background:`${primary}15`,border:`1px solid ${primary}30`,borderRadius:br?.full||'9999px',padding:'3px 10px',fontSize:fs(typo,'xs'),color:primary,fontWeight:fw(typo,'semibold')}}>
              <MdAccessTime size={11}/>{s.fecha} · {slotToHHMM(s.slot)}
            </span>
          ))}
        </div>
        ...
        <div style={{overflowY:'auto',flex:1,padding:`${sp?.lg||'24px'} ${sp?.xl||'32px'}`}}>
          {exitoso?(
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:sp?.xl||'32px',gap:sp?.md||'16px',minHeight:'200px'}}>
              <div style={{width:'60px',height:'60px',borderRadius:br?.full||'9999px',background:colors?.success?.light||'#d1fae5',display:'flex',alignItems:'center',justifyContent:'center'}}>
                <MdCheck size={34} style={{color:colors?.success?.main||'#10b981'}}/>
              </div>
              <p style={{margin:0,fontSize:fs(typo,'md'),fontWeight:fw(typo,'semibold'),color:colors?.success?.dark||'#059669',textAlign:'center'}}>
                ¡Cita{slots.length>1?'s':''} agendada{slots.length>1?'s':''} correctamente!
              </p>
            </div>
          ):loadingData?(
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',padding:sp?.xl||'32px'}}>
              <div style={{width:'28px',height:'28px',border:`3px solid ${colors?.neutral?.[200]||'#e5e7eb'}`,borderTop:`3px solid ${primary}`,borderRadius:'50%',animation:'spin 1s linear infinite'}}/>
            </div>
          ):(
              ...
              <div>
                ...
                <div style={{border:`1px solid ${colors?.neutral?.[200]||'#e5e7eb'}`,borderRadius:br?.lg||'8px',height:'160px',overflowY:'auto',background:colors?.neutral?.[0]||'#fff'}}>
                  {filtrados.length===0?(
                    ...
                    return(
                      ...
                      >
                        <div>
                          <div style={{fontSize:fs(typo,'sm'),fontWeight:sel?fw(typo,'semibold'):fw(typo,'normal'),color:sel?primary:colors?.neutral?.[800]||'#1f2937'}}>{p.nombre} {p.apellido}</div>
                ...
                {pacienteSel&&(
                  <div style={{marginTop:sp?.xs||'4px',display:'inline-flex',alignItems:'center',gap:'6px',background:`${primary}10`,border:`1px solid ${primary}30`,borderRadius:br?.full||'9999px',padding:'3px 10px',fontSize:fs(typo,'xs'),color:primary,fontWeight:fw(typo,'semibold')}}>
                    <MdCheck size={11}/> {pacienteSel.nombre} {pacienteSel.apellido}
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {tipoCitaSel&&(
                  <div style={{marginTop:sp?.sm||'8px',padding:sp?.md||'16px',background:`${primary}08`,border:`1px solid ${primary}20`,borderRadius:br?.md||'6px'}}>
                    {tipoCitaSel.descripcion&&<p style={{margin:`0 0 6px`,fontSize:fs(typo,'xs'),color:colors?.neutral?.[600]||'#4b5563'}}>{tipoCitaSel.descripcion}</p>}
                    <div style={{display:'flex',gap:'16px',flexWrap:'wrap',fontSize:fs(typo,'xs'),color:primary,fontWeight:fw(typo,'semibold')}}>
                      <span><MdAccessTime size={11} style={{verticalAlign:'middle'}}/> Duración tipo: {tipoCitaSel.duracion_promedio} min</span>
                      <span>Slots seleccionados: {slots.length} × 30 min = {slots.length*30} min</span>
                      {tipoCitaSel.costo_base&&<span>$ {parseFloat(tipoCitaSel.costo_base).toFixed(2)}</span>}
                    </div>
                  </div>
                )}
              </div>
        ...
        {!exitoso&&(
          <div style={{padding:`${sp?.md||'16px'} ${sp?.xl||'32px'}`,borderTop:`1px solid ${colors?.neutral?.[200]||'#e5e7eb'}`,display:'flex',justifyContent:'flex-end',gap:sp?.sm||'8px',background:colors?.neutral?.[50]||'#f9fafb',flexShrink:0}}>
            <button onClick={()=>!guardando&&onClose()} style={{padding:`${sp?.sm||'8px'} ${sp?.lg||'24px'}`,background:colors?.neutral?.[100]||'#f3f4f6',border:`1px solid ${colors?.neutral?.[200]||'#e5e7eb'}`,borderRadius:br?.md||'6px',fontSize:fs(typo,'sm'),fontWeight:fw(typo,'semibold'),color:colors?.neutral?.[700]||'#374151',cursor:guardando?'not-allowed':'pointer'}}>
              Cancelar
            </button>
...
const CitasDashboard = ({doctorId})=>{
  ...
  const [slotsSet, setSlotsSet] = useState(new Set()); // keys: JSON({fecha,slot})
  ...
  const toggleSlot=(fecha,slot)=>{
    if(esPasadoSlot(fecha,slot)) return;
    const key=JSON.stringify({fecha,slot});
  ...
  const slotsSeleccionados = Array.from(slotsSet)
    .map(k=>JSON.parse(k))
    .sort((a,b)=>a.fecha===b.fecha?a.slot-b.slot:a.fecha.localeCompare(b.fecha));

  // ── Confirmar: una cita por cada slot de 30 min ──
  const handleConfirmarCita=async({pacienteId,tipoCitaId,notas,slots})=>{
    const id=getId();
    if(!id) throw new Error('No se encontró el ID del doctor');

    await Promise.all(slots.map(s=>{
      const h=Math.floor(s.slot);
      const m=s.slot%1===0.5?'30':'00';
      const fecha_hora=`${s.fecha} ${String(h).padStart(2,'0')}:${m}:00`;
      return api.post('/citas',{
        id_usuario:       parseInt(id),
        id_paciente:      parseInt(pacienteId),
        id_tipo_cita:     parseInt(tipoCitaId),
        fecha_hora,
        duracion_minutos: 30,          // ✅ fijo 30 min por slot
        notas_previa:     notas||null,
        estado:           'programada',
      });
    }));
    ...
    return(
      <div style={card}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',borderBottom:`2px solid ${colors?.neutral?.[200]||'#e5e7eb'}`}}>
          {DIAS_C.map(d=>(
            <div key={d} style={{padding:'10px 0',textAlign:'center',fontSize:fs(typo,'xs'),fontWeight:fw(typo,'semibold'),color:colors?.neutral?.[500]||'#6b7280',letterSpacing:'0.06em',textTransform:'uppercase',borderRight:`1px solid ${colors?.neutral?.[100]||'#f3f4f6'}`}}>{d}</div>
          ))}
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)'}}>
          {dias.map((dia,idx)=>{
            ...
            return(
              <div key={key} onClick={()=>{if(pasado)return;citasD.length>0?irADia(dia):irASemana(dia);}}
  ...
  // ════════════════════════════════════════════════════════════════
  // VISTA HORARIA — slots de 30 min con estilo mejorado
  // ════════════════════════════════════════════════════════════════
    ...
    return(
      <div style={card}>
        ...
        <div
          style={{
            display: "flex",
            borderBottom: `3px solid ${colors?.neutral?.[200] || "#e5e7eb"}`,
        ...
        >
          <div
            style={{
              width: "68px",
              flexShrink: 0,
              borderRight: `3px solid ${colors?.neutral?.[200] || "#e5e7eb"}`,
            }}
          />
          {diasVista.map((d, i) => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  padding: "10px 4px",
                  borderRight:
                    i < diasVista.length - 1
                      ? `3px solid ${colors?.neutral?.[200] || "#e5e7eb"}`
                      : "none",
              ...
              >
                ...
                <div
                  style={{
                    ...
                    width: "32px",
                    height: "32px",
                    borderRadius: br?.full || "9999px",
                    marginTop: "3px",
                    background: esHoy ? primary : "transparent",
                    color: esHoy
                      ? "#fff"
                      : pasado
                    ...
                    fontSize: fs(typo, "md"),
                    fontWeight: esHoy ? fw(typo, "bold") : fw(typo, "normal"),
                  }}
                >
                  {d.getDate()}
                </div>
          ...
          <div style={{ display: "flex" }}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      color: esHoraEnPunto
                        ...
                        : colors?.neutral?.[300] || "#d1d5db",
                      fontWeight: esHoraEnPunto
                        ? fw(typo, "semibold")
                        : fw(typo, "normal"),
                      borderBottom: esHoraEnPunto
                        ? `3px solid ${colors?.neutral?.[200] || "#e5e7eb"}`
                        : `3px dashed ${colors?.neutral?.[100] || "#f3f4f6"}`,
                      userSelect: "none",
            ...
            {diasVista.map((d, di) => {
              const dateKey = toDateStr(d),
                citasD = citasPorFecha[dateKey] || [],
                colPas = esPasado(dateKey);
              return (
                <div
                  key={di}
                  style={{
                ...
                >
                  {/* Slots de 30 min */}
                  {SLOTS.map((slot) => {
                    const slotKey = JSON.stringify({ fecha: dateKey, slot });
                    const activo = slotsSet.has(slotKey);
                    const bloqueado = esPasadoSlot(dateKey, slot);
                    const esHoraEnPunto = slot % 1 === 0;

                    return (
                      <div
                        key={slot}
                        onClick={() => !bloqueado && toggleSlot(dateKey, slot)}
                        style={{
                          height: `${SLOT_H}px`,
                          borderBottom: esHoraEnPunto
                            ? `3px solid ${colors?.neutral?.[200] || "#e5e7eb"}`
                            : `3px dashed ${colors?.neutral?.[100] || "#f3f4f6"}`,
                          background: bloqueado ? "transparent" : activo
                            ? `${primary}20`
                            : "transparent",
                  ...
                  {citasD.map((c, ci) => {
                    ...
                    const dur = c.duracion_minutos || c.TipoCita?.duracion_promedio || 30;
                    const h = Math.max((dur / 60) * HORA_H - 3, SLOT_H - 3);
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {cd.toLocaleTimeString("es", { hour: "2-digit", minute: "2-digit" })} {c.Paciente?.nombre || ""}
                        </div>
                        {h > 28 && (
                          <div
                            style={{
    ...
    <div style={{ minHeight: '100vh', background: colors?.neutral?.[50] || '#f9fafb', padding: sp?.xl || '32px' }}>
        ...
        <div style={{ marginBottom: sp?.md||'16px', display:'flex', flexWrap:'wrap', gap: sp?.xs||'4px' }}>
          {slotsSeleccionados.map((s,i)=>(
            <span key={i} style={{display:'inline-flex',alignItems:'center',gap:'4px',background:`${primary}12`,border:`1px solid ${primary}30`,borderRadius:br?.full||'9999px',padding:'4px 12px',fontSize:fs(typo,'xs'),color:primary,fontWeight:fw(typo,'semibold')}}>
              <MdAccessTime size={11}/> {s.fecha} · {slotToHHMM(s.slot)} (30 min)
            </span>
          ))}
        </div>
      )}
      ...
      {vista==='mes'&&(
        <div style={{ marginBottom: sp?.sm||'8px', fontSize: fs(typo,'xs'), color: colors?.neutral?.[400]||'#9ca3af', display:'flex', alignItems:'center', gap:'6px' }}>
          <MdCalendarToday size={11}/> Haz clic en un día para ver la agenda · Días pasados no disponibles
        </div>
      )}
